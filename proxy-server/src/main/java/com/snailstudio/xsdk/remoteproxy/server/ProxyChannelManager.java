...
import com.snailstudio.xsdk.remoteproxy.protocol.Constants;
...
public class ProxyChannelManager {
    ...
    public static void removeCmdChannel(Channel channel) {
        ...
        if (channel != channel0) {
            // Modify by xuqiqiang 2018.1.10
            // Fix bug: Clients cannot reconnect when delete clientKey then add it.
            closeChannel(channel0);
//            cmdChannels.put(clientKey, channel);
        }
...
//                portCmdChannelMapping.put(port, proxyChannel);
    ...
    private static void closeChannel(Channel channel) {
        try {
            ...
            logger.info("error when disconnect user channel {}", e);
        }
    }
